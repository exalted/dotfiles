...
#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysqld" --initialize-insecure --user=ali --basedir="$(brew --prefix)/opt/mysql@8.0" --datadir="$(brew --prefix)/var/mysql@8.0" --tmpdir=/tmp'

#   # TODO: This is possibly buggy, because if a MySQL server is already running this will fail to start:
#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysqld_safe" --datadir="$(brew --prefix)/var/mysql@8.0" &'
#   sleep 5 # MySQL might take a few seconds to start

#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysql" -u root --execute="update mysql.user set plugin = \'mysql_native_password\';"'
#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysql" -u root --execute="DROP USER \'mysql.infoschema\'@\'localhost\';"'
#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysql" -u root --execute="CREATE USER \'mysql.infoschema\'@\'localhost\';"'
#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysql" -u root --execute="UPDATE mysql.user SET max_questions = 0, max_updates = 0, max_connections = 0 WHERE User = \'mysql.infoschema\' AND Host = \'localhost\';"'
#   system '"$(brew --prefix)/opt/mysql@8.0/bin/mysql" -u root --execute="GRANT CREATE USER, DELETE, SELECT, EXECUTE, CREATE ROUTINE, ALTER ROUTINE, GRANT OPTION, REFERENCES, CREATE VIEW, TRIGGER, FILE, UPDATE, DROP, CREATE, REPLICATION CLIENT, REPLICATION SLAVE, CREATE TABLESPACE, LOCK TABLES, EVENT, INDEX, ALTER, SHOW VIEW, RELOAD, SHOW DATABASES, INSERT, SHUTDOWN, SUPER, PROCESS, CREATE TEMPORARY TABLES ON *.* TO \'mysql.infoschema\'@\'localhost\';"'
